...
class LifxBackend {
    ...
    async execute(commandName, payload = {}) {
        switch (commandName) {
            ...
            case 'setColorTemp': {
                const kelvin = Math.max(1500, Math.min(9000, Number.parseInt(payload.kelvin, 10) || this.defaultKelvin));
                const hsbk = { h: 0, s: 0, b: this._brightnessToLifx(payload.brightness ?? 100), k: kelvin };
                await this._sendMessage(this._buildSetColorMessage(hsbk));
                return { applied: true };
            }
            case 'fade': {
                ...
                if (bri === 0) {
                    ...
                    const hsbk = { h: 0, s: 0, b: this._brightnessToLifx(bri), k: this.defaultKelvin };
                    await this._sendMessage(this._buildSetColorMessage(hsbk, duration));
                }
                return { applied: true };
            }
            default:
