...
// WiZ uses direct R/G/B (0–255) over UDP for colour, and a native `temp` field
// (kelvin, 2200–6500) for white/warm/cool scenes — driving the dedicated tungsten
// and cool-white LEDs directly instead of mixing RGB channels.
 ...
 * `temp` is colour temperature in kelvin (2200–6500) for the white LED channel.
    ...
    async execute(commandName, payload = {}) {
        switch (commandName) {
            ...
            case 'setColorTemp': {
                ...
                return this._sendPilot({ state: true, temp, dimming: this._clampBrightness(payload.brightness ?? 100) });
            }
            case 'fade': {
