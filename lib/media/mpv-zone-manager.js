...
class MpvProfileManager {
    ...
    buildMpvArgs(profileName, audioDevice, targetMonitor) {
        ...
        // Handle fullscreen vs windowed mode based on profile
        const fullscreen = profile.fullscreen !== undefined ? profile.fullscreen : true; // Default to fullscreen for backward compatibility
        
        if (fullscreen) {
            mpvArgs.push('--fullscreen');
            this.logger.info('Using fullscreen mode');
        } else {
            // Windowed mode with geometry
            const geometry = profile.windowGeometry || '960x540'; // Default to 50% of 1080p
            mpvArgs.push(`--geometry=${geometry}`);
            this.logger.info(`Using windowed mode with geometry: ${geometry}`);
            
            // Remove --ontop for windowed mode to allow better window management
