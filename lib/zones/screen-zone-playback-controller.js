...
class ScreenZonePlaybackController {
    ...
    async setDefaultImage() {
        ...
        for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
            try {
                await setDefault();
                if (attempt > 1) {
                    zone.logger.info(`Default image set succeeded on attempt ${attempt}/${maxAttempts}: ${zone.defaultImage}`);
                }
                return;
            } catch (error) {
                const message = (error && error.message) ? error.message : String(error);
                const isInitRace = /MPV Zone Manager not initialized/i.test(message);

                if (isInitRace && attempt < maxAttempts) {
                    if (attempt === 1) {
                        zone.logger.warn(`Default image set raced MPV init for ${zone.defaultImage}; retrying up to ${maxAttempts} attempts`);
                    }
                    await new Promise(resolve => setTimeout(resolve, retryDelayMs));
                    continue;
                }

                zone.logger.warn(`Failed to set default image ${zone.defaultImage}:`, message);
                return;
            }
        }
    }
    ...
    async playVideo(command) {
        ...
        if (shouldResume) {
            ...
            await zone.mpvZoneManager.loadMedia(fullPath, 'video', options);
            await zone.mpvZoneManager.play();
            zone.smartMediaState.lastCommand = 'playVideo';
