...
class ScreenZone extends BaseZone {
    constructor(config, mqttClient, zoneManager) {
        ...
        this.monitorControlMethod = ScreenPowerManager.normalizeMethod(
            config.monitorControlMethod || config.monitor_control_method || 'none'
        );
        this.monitorCecDevice = config.monitorCecDevice || config.monitor_cec_device || null;
        this.monitorI2cBus = config.monitorI2cBus != null ? config.monitorI2cBus : (config.monitor_i2c_bus != null ? config.monitor_i2c_bus : null);
        this.outputName = config.outputName || config.output_name || null;
        ...
        try {
            switch (command.command) {
                ...
                case 'playSpeech': {
                    ...
                    await this._playSpeech(command.file, command.volume, command.ducking, { adjustVolume: command.adjustVolume, id: command.id });
                    break;
                }
                case 'pauseSpeech':
                ...
                case 'resumeSpeech':
                    ...
                    break;
                case 'skipSpeech': {
                    const skipResult = await this.audioManager.skipSpeech({ id: command.id, file: command.file });
    ...
    getSupportedCommands() {
        return [
            ...
            'playSpeech', 'pauseSpeech', 'resumeSpeech', 'skipSpeech', 'speechQueue', 'audioStatus', 'playAudioFX', 'playSoundEffect', 'stopAudio', 'stopSpeech',
            'sleepScreen', 'wakeScreen', 'recoverScreens',
            'reboot', 'shutdown', 'killPfx', 'restartPfx', 'restart',
            'switchMedia', 'start',
    ...
    async _stopBackgroundMusic(fadeTime = 0, selector = {}) {
        return this._stopBackgroundWithFade(fadeTime, {
            ...
            resetState: () => {
                ...
                this.currentState.backgroundMusic = this._derivedBackgroundMusicFile(snap);
    ...
    async _pauseBackgroundMusic(selector = {}) {
        return this._pauseBackgroundWithLifecycle({
            selector,
    ...
    async _resumeBackgroundMusic(selector = {}) {
        return this._resumeBackgroundWithLifecycle({
            selector,
    ...
    async _playSpeech(audioPath, volume, ducking, opts = {}) {
        ...
        try {
            await this.audioManager.playSpeech(fileValidation.path, { volume: targetVolume, id: opts.id, displayFile: audioPath });
        } catch (err) {
            playbackError = err;
    ...
    _monitorControlOptions() {
        return {
            method: this.monitorControlMethod,
            targetMonitor: this.targetMonitor,
            outputName: this.outputName,
            cecDevice: this.monitorCecDevice,
            i2cBus: this.monitorI2cBus
        };
    }

    /**
     * Best-effort wake via configured monitor_control_method before media.
     * No-op when method is none. Failures are logged; media still proceeds.
     */
    async _autoWakeForMedia(mediaType = 'media') {
        // Only setImage ('image') and playVideo ('video') reach here - audio commands never do.
        // Visual media must own the screen, so raise MPV above the browser before playing.
        // Must run before the early returns below, or the screen stays on the browser.
        if (this.browserManager && this.browserManager.enabled) {
            try {
                await this.browserController.switchToMpv();
            } catch (error) {
                this.logger.warn(`Auto-hide browser for ${mediaType} failed: ${error.message}`);
            }
        }
        if (this.monitorControlMethod === 'none') {
            return;
        }
        // A CEC/DDC round-trip costs several seconds and blocks playback. The screen is
        // only asleep after an explicit sleepScreen, so skip the wake when already awake.
        if (this.currentState.screenAwake) {
            this.logger.debug(`Auto-wake for ${mediaType} skipped: screen already awake`);
            return;
        }
        try {
            this.logger.info(`Auto-waking display for ${mediaType} via ${this.monitorControlMethod}`);
            await this.screenPowerManager.wakeOutput(this._monitorControlOptions());
            this.currentState.screenAwake = true;
        } catch (error) {
            this.logger.warn(`Auto-wake for ${mediaType} failed: ${error.message}`);
        }
    }
    ...
    async _sleepScreen() {
        ...
        const result = await this.screenPowerManager.sleepOutput(this._monitorControlOptions());
        if (!result.applied) {
            this.logger.info(`sleepScreen no-op (monitor_control_method=${result.method})`);
            this.publishEvent({
                screen_sleep: false,
                ignored: true,
                reason: result.reason || result.method
            });
            return;
        }
        ...
        this.publishStatus();
        this.publishEvent({ screen_sleep: true, method: result.method });
        this.logger.info(`Screen put to sleep via ${result.method}`);
    }

    async _wakeScreen() {
        const result = await this.screenPowerManager.wakeOutput(this._monitorControlOptions());
        if (!result.applied) {
            this.logger.info(`wakeScreen no-op (monitor_control_method=${result.method})`);
            this.publishEvent({
                screen_wake: false,
                ignored: true,
                reason: result.reason || result.method
        ...
        this.publishStatus();
        this.publishEvent({ screen_wake: true, method: result.method });
        this.logger.info(`Screen woken up via ${result.method}`);
    }
    ...
    async _recomputeBackgroundAfterDuckChange() {
        await this._recomputeBackgroundAfterDuckChangeShared({
            hasBackgroundPlayback: () => Boolean(this.currentState.backgroundMusic) || (this.audioManager && typeof this.audioManager.hasActiveBackground === 'function' && this.audioManager.hasActiveBackground()),
    ...
    /**
     * Reload currently active default/idle still and looping video from the new pack.
     * @protected
     */
    async _refreshVisualMediaPackLayers() {
        const refreshed = [];
        const looping = this.loopState && this.loopState.isLooping && this.loopState.currentVideoFile;
        if (looping) {
            await this._playVideo({ file: this.loopState.currentVideoFile, loop: true });
            refreshed.push('video');
        } else if (this.currentState.currentImage && (this.currentState.status === 'showing_image' || !this.currentState.currentVideo)) {
            await this._setImage({ file: this.currentState.currentImage });
            refreshed.push('default_image');
        }
        return refreshed;
    }
