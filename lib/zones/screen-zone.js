...
class ScreenZone extends BaseZone {
    ...
    _resolveDeviceMediaDir(config) {
        ...
        if (deviceMediaDir && path.isAbsolute(deviceMediaDir)) return deviceMediaDir;
        // Use default base path for relative media_dir values; do not rely on media_base_path
    ...
    async _playVideo(command) {
        ...
        if (isVideo && duckingLevel < 0) {
            ...
            this.logger.debug(`Video duck trigger added: ${duckId} level=${duckingLevel}`);
        }
    ...
    async _processVideoQueue() {
        ...
        try {
            ...
            if (kind === 'playVideo') {
                ...
                if (media_type === 'video') {
                    // Normal path waits for unified completion event to clear isProcessing.
                    // If playback never started (e.g., validation failure/early return),
                    // release queue processing so later video commands are not deadlocked.
                    if (this.currentState.status !== 'playing_video') {
                        this.logger.warn(`VIDEO_QUEUE_PROCESS: playVideo did not enter playing_video state for ${file}; releasing queue lock`);
                        this.isProcessingVideoQueue = false;
                        if (this.videoQueue.length > 0) this._processVideoQueue();
                        return;
                    }
    ...
    async _enableBrowser(url = 'http://localhost/clock/') {
        if (this.browserManager.enabled) {
            if (!this.browserManager.windowId) {
                this.logger.warn('Browser already enabled but windowId is null, retrying detection');
                this.browserManager.windowId = this.windowManager.findChromiumWindowId(this.browserManager.className);
                if (!this.browserManager.windowId) {
                    this.browserManager.windowId = await this.windowManager.waitForWindowByClass(this.browserManager.className, 3000);
                }
                if (this.browserManager.windowId) {
                    this.logger.info(`Browser window recovered: ${this.browserManager.windowId}`);
                } else {
                    this.logger.warn('Browser window still not found after retry');
                }
            } else {
        ...
        this.logger.debug(`enableBrowser result: pid=${this.browserManager.process?.pid || 'unknown'}, windowId=${this.browserManager.windowId || 'not-found'}`);
        ...
        if (this.browserManager.windowId) {
            try {
                ...
                if (found && found !== this.browserManager.windowId) {
                    this.logger.debug(`enableBrowser: detected chromium windowId changed (stored=${this.browserManager.windowId} detected=${found}), updating stored id`);
        ...
        if (this.browserManager.windowId) {
            ...
            if (mpvWindow) {
                ...
                this.logger.debug('MPV window not found - browser may be visible');
            }
        } else {
    ...
    async _showBrowser() {
        ...
        if (!this.browserManager.windowId) {
            this.logger.warn('showBrowser: windowId is null, attempting recovery detection');
            this.browserManager.windowId = this.windowManager.findChromiumWindowId(this.browserManager.className);
            if (!this.browserManager.windowId) {
                this.browserManager.windowId = await this.windowManager.waitForWindowByClass(this.browserManager.className, 3000);
            }
            ...
            this.logger.info(`showBrowser: recovered browser windowId=${this.browserManager.windowId}`);
        }

        this.logger.debug(`Showing browser (pure window management) - Window ID: ${this.browserManager.windowId}`);
        ...
        try {
            let t0 = Date.now();
            try {
                const res = this.windowManager.addWindowState(browserWinRefreshed, 'above');
            ...
            try {
                t0 = Date.now();
                const resAct = this.windowManager.activateWindow(browserWinRefreshed);
            ...
            try {
                becameActive = this.windowManager.isWindowActive(browserWinRefreshed);
            } catch (err) {
            ...
            if (!becameActive) {
                ...
                for (let attempt = 1; attempt <= 3 && !becameActive; attempt++) {
                    try {
                        ...
                        const t1 = Date.now();
                        try { this.windowManager.activateWindow(browserWinRefreshed); } catch (e) { this.logger.debug(`retry activateWindow attempt ${attempt} failed: ${e.message}`); }
                        ...
                        if (this.windowManager.wmctrlActivate) {
                            try {
                                const t2 = Date.now();
                                this.windowManager.wmctrlActivate(browserWinRefreshed);
        ...
        // Give X11 window manager a moment to fully settle the focus state before publishing,
        // otherwise _updateFocusAndContent() might see the window as inactive and revert focus
        await new Promise(r => setTimeout(r, 150));

        // Rebind to the actual active Chromium window to avoid stale-id focus mismatches.
        try {
            const activeRaw = execSync('xdotool getactivewindow', { env: { ...process.env, DISPLAY: this.display } }).toString().trim();
            if (activeRaw) {
                let activeClass = '';
                try {
                    activeClass = execSync(`xdotool getwindowclassname ${activeRaw}`, { env: { ...process.env, DISPLAY: this.display } }).toString().trim();
                } catch (_) {
                    activeClass = '';
                }
                if (activeClass && activeClass === this.browserManager.className) {
                    if (String(this.browserManager.windowId) !== String(activeRaw)) {
                        this.logger.info(`_showBrowser: rebinding browser windowId from ${this.browserManager.windowId} to active window ${activeRaw} (${activeClass})`);
                        this.browserManager.windowId = activeRaw;
                    }
                }
            }
        } catch (e) {
            this.logger.debug(`_showBrowser: active-window rebinding skipped: ${e.message}`);
        }
    ...
    async _hideBrowser() {
        ...
        this.logger.debug(`Hiding browser (pure window management) - stored windowId=${this.browserManager.windowId || 'none'}`);
    ...
    _updateFocusAndContent() {
        ...
        // Primary check: stored browser window id.
        if (this.browserManager.windowId && this.windowManager.isWindowActive(this.browserManager.windowId)) {
            browserForeground = true;
        }

        // Fallback: match active window to browser PID to recover from stale window id.
        if (!browserForeground && this.browserManager.process?.pid) {
            try {
                const targetPid = Number(this.browserManager.process.pid);
                const activeRaw = execSync('xdotool getactivewindow', { env: { ...process.env, DISPLAY: this.display } }).toString().trim();
                const activeNorm = normalizeWindowId(activeRaw);
                if (activeNorm) {
                    const wmPidOut = execSync('wmctrl -lp', { env: { ...process.env, DISPLAY: this.display } }).toString();
                    const lines = wmPidOut.split('\n');
                    for (const line of lines) {
                        const m = line.trim().match(/^(0x[0-9a-fA-F]+)\s+\S+\s+(\d+)/);
                        if (!m) continue;
                        const winNorm = normalizeWindowId(m[1]);
                        const pid = Number(m[2]);
                        if (winNorm && winNorm === activeNorm && pid === targetPid) {
                            browserForeground = true;
                            if (String(this.browserManager.windowId) !== String(m[1])) {
                                this.logger.info(`_updateFocusAndContent: rebinding stale browser windowId ${this.browserManager.windowId} -> ${m[1]} via active PID match`);
                                this.browserManager.windowId = m[1];
                            }
                            break;
                        }
                    }
                ...
                this.logger.debug(`_updateFocusAndContent PID fallback failed: ${err.message}`);
            }
        }
