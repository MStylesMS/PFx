...
class BaseZone {
    ...
    _resolveMediaPath(filename) {
        const path = require('path');
        // Resolve media path using only media_dir semantics.
        // Rules:
        // - If mediaDir is absolute -> treat it as the resolved directory root.
        // - Otherwise -> join default base '/opt/paradox/media' with mediaDir (may be empty).
        const deviceMediaDir = this.config.mediaDir || this.config.media_dir || '';
        if (deviceMediaDir && path.isAbsolute(deviceMediaDir)) {
            return path.resolve(path.join(deviceMediaDir, filename));
        }
        const mediaBase = '/opt/paradox/media';
        return path.resolve(path.join(mediaBase, deviceMediaDir, filename));
    }
