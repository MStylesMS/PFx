...
class ScreenZoneBrowserController {
    ...
    // xdotool only gained `getwindowclassname` in 3.20211022 (Trixie); Bookworm ships 3.20160805, so match via class search instead
    _windowMatchesClass(windowId, className) {
        const target = this._normalizeWindowId(windowId);
        if (!target || !className) return false;
        try {
            const out = execSync(`xdotool search --class ${className}`, {
                env: this._env(),
                stdio: ['ignore', 'pipe', 'pipe']
            }).toString();
            return out.split('\n').some(line => this._normalizeWindowId(line) === target);
        } catch (error) {
            return false;
        }
    }

    async enableBrowser(url = null) {
        const zone = this.zone;

        if (!url) {
            zone.logger.warn('enableBrowser called with no URL — set browser_url in the zone INI config');
            return;
        }
    ...
    async showBrowser() {
        ...
        try {
            ...
            if (activeRaw && this._windowMatchesClass(activeRaw, zone.browserManager.className)) {
                if (String(zone.browserManager.windowId) !== String(activeRaw)) {
                    zone.logger.info(`_showBrowser: rebinding browser windowId from ${zone.browserManager.windowId} to active window ${activeRaw} (class ${zone.browserManager.className})`);
            ...
            if (zone.browserManager.enabled && zone.browserManager.keepAlive && zone.browserManager.process) {
                try {
                    ...
                    if (error.code === 'ESRCH') {
                        ...
                        const previousUrl = zone.browserManager.url || null;
                        if (!previousUrl) {
                            zone.logger.warn('Browser crashed but no URL configured — skipping keep-alive restart');
                            return;
                        }
                        const wasFocused = zone.currentState.focus === 'chromium';
