...
class ScreenZoneVideoQueueController {
    ...
    async handleLoopRestart() {
        ...
        try {
            ...
            const resolvedLoopPath = zone._resolveMediaPath(videoPath);
            await withTimeout(zone.mpvZoneManager.loadMedia(resolvedLoopPath, 'video'), 3000, 'MPV loadMedia');
            await withTimeout(zone.mpvZoneManager.play(), 2000, 'MPV play');

            const probedDuration = await probeDurationSeconds(resolvedLoopPath).catch(() => null);
