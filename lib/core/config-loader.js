...
class ConfigLoader {
    ...
    async _load(filePath, options = {}) {
        try {
            ...
            rawConfig = await this._mergeLightsConfigIfPresent(filePath, rawConfig, options);
    ...
    async _mergeLightsConfigIfPresent(mainConfigPath, rawConfig, options = {}) {
        const globalSection = rawConfig.global || {};
        const configuredLightsPath = options.lightsConfigPath || globalSection.lights_config || globalSection.LIGHTS_CONFIG;

        if (!configuredLightsPath) {
            return rawConfig;
        }

        const lightsConfigPath = path.isAbsolute(configuredLightsPath)
            ? configuredLightsPath
            : path.resolve(path.dirname(mainConfigPath), configuredLightsPath);

        this.logger.info(`Loading supplemental lights configuration from ${lightsConfigPath}`);
        const lightsContent = await fs.readFile(lightsConfigPath, 'utf8');
        const lightsRawConfig = ini.parse(lightsContent);

        for (const [sectionName, sectionData] of Object.entries(lightsRawConfig)) {
            if (sectionName === 'global' || sectionName === 'mqtt') {
                this.logger.warn(`Ignoring [${sectionName}] section from supplemental lights config`);
                continue;
            }
    ...
    _processDeviceConfig(deviceName, config) {
        ...
        switch (deviceType) {
            ...
            case 'light':
            case 'light_group':
                processed.controller = config.controller || config.CONTROLLER;
                processed.deviceId = config.device_id || config.DEVICE_ID || config.node_id || config.NODE_ID || null;
                processed.deviceList = this._parseListValue(
                    config.device_list || config.DEVICE_LIST || config.devices || config.DEVICES || config.lights || config.LIGHTS
                );
                processed.backend = (config.backend || config.BACKEND || '').toString().trim().toLowerCase() || 'passthrough';
                processed.generation = `${config.generation || config.GENERATION || config.shelly_generation || config.SHELLY_GENERATION || ''}`.trim() || undefined;
                processed.profile = (config.profile || config.PROFILE || config.device_profile || config.DEVICE_PROFILE || '').toString().trim().toLowerCase() || undefined;
                processed.model = (config.model || config.MODEL || config.device_model || config.DEVICE_MODEL || '').toString().trim() || undefined;
                processed.forwardTopic = config.forward_topic || config.FORWARD_TOPIC || null;
                processed.bulbIp = config.bulb_ip || config.BULB_IP || config.light_ip || config.LIGHT_IP || config.ip || config.IP || null;
                processed.bulbIps = this._parseListValue(config.bulb_ips || config.BULB_IPS || config.ips || config.IPS);
                processed.targetHosts = this._parseListValue(config.target_hosts || config.TARGET_HOSTS || config.hosts || config.HOSTS);
                processed.wizPort = parseInt(config.wiz_port || config.WIZ_PORT, 10) || undefined;
                processed.sceneMap = this._parseSceneMap(config.scene_map || config.SCENE_MAP);
                processed.shellyHost = config.shelly_host || config.SHELLY_HOST || config.host || config.HOST || null;
                processed.shellyAuthUser = config.shelly_auth_user || config.SHELLY_AUTH_USER || config.username || config.USERNAME || null;
                processed.shellyAuthPass = config.shelly_auth_pass || config.SHELLY_AUTH_PASS || config.password || config.PASSWORD || null;
                processed.shellyChannel = parseInt(config.channel || config.CHANNEL || config.shelly_channel || config.SHELLY_CHANNEL, 10);
                processed.shellyRpcMethod = config.shelly_rpc_method || config.SHELLY_RPC_METHOD || null;
                processed.httpTimeoutMs = parseInt(config.http_timeout_ms || config.HTTP_TIMEOUT_MS || config.shelly_timeout_ms || config.SHELLY_TIMEOUT_MS, 10) || undefined;
                // Hue v2 fields
                processed.hueBridgeHost = config.hue_bridge_host || config.HUE_BRIDGE_HOST || null;
                processed.hueAppKey = config.hue_app_key || config.HUE_APP_KEY || null;
                processed.hueResourceId = config.hue_resource_id || config.HUE_RESOURCE_ID || null;
                processed.hueResourceType = (config.hue_resource_type || config.HUE_RESOURCE_TYPE || 'room').toString().trim().toLowerCase();
                processed.hueProfile = (config.hue_profile || config.HUE_PROFILE || 'color').toString().trim().toLowerCase();
                // LIFX LAN fields
                processed.lifxPort = parseInt(config.lifx_port || config.LIFX_PORT, 10) || undefined;
                processed.lifxKelvin = parseInt(config.lifx_kelvin || config.LIFX_KELVIN, 10) || undefined;
                // Z-Wave direct backend fields
                processed.zwaveMode = (config.zwave_mode || config.ZWAVE_MODE || 'direct').toString().trim().toLowerCase();
                processed.zwavePort = config.zwave_port || config.ZWAVE_PORT || null;
                processed.zwaveNodeId = parseInt(config.zwave_node_id || config.ZWAVE_NODE_ID || config.node_id || config.NODE_ID, 10) || undefined;
                processed.zwaveType = (config.zwave_type || config.ZWAVE_TYPE || '').toString().trim().toLowerCase() || undefined;
                processed.zwavePollMs = parseInt(config.zwave_poll_ms || config.ZWAVE_POLL_MS, 10) || undefined;
                processed.zwaveSecurityMode = (config.zwave_security_mode || config.ZWAVE_SECURITY_MODE || 'none').toString().trim().toLowerCase();
                // Zigbee direct backend fields
                processed.zigbeeMode = (config.zigbee_mode || config.ZIGBEE_MODE || 'direct').toString().trim().toLowerCase();
                processed.zigbeePort = config.zigbee_port || config.ZIGBEE_PORT || null;
                processed.zigbeeAdapter = (config.zigbee_adapter || config.ZIGBEE_ADAPTER || 'ember').toString().trim().toLowerCase();
                processed.zigbeeIeee = config.zigbee_ieee || config.ZIGBEE_IEEE || null;
                processed.zigbeeType = (config.zigbee_type || config.ZIGBEE_TYPE || '').toString().trim().toLowerCase() || undefined;
                processed.zigbeeDbPath = config.zigbee_db_path || config.ZIGBEE_DB_PATH || null;
                break;

            case 'input':
                processed.backend = (config.backend || config.BACKEND || 'generic').toString().trim().toLowerCase();
                processed.generation = `${config.generation || config.GENERATION || config.shelly_generation || config.SHELLY_GENERATION || ''}`.trim() || undefined;
                processed.profile = (config.profile || config.PROFILE || 'input').toString().trim().toLowerCase();
                processed.model = (config.model || config.MODEL || '').toString().trim() || undefined;
                processed.inputTopic = config.input_topic || config.INPUT_TOPIC || config.topic || config.TOPIC || null;
                processed.inputTopics = this._parseListValue(config.input_topics || config.INPUT_TOPICS || config.topics || config.TOPICS);
                processed.inputMap = this._parseJsonValue(config.input_map || config.INPUT_MAP, 'input_map') || {};
                processed.shellyHost = config.shelly_host || config.SHELLY_HOST || config.host || config.HOST || null;
                processed.shellyAuthUser = config.shelly_auth_user || config.SHELLY_AUTH_USER || config.username || config.USERNAME || null;
                processed.shellyAuthPass = config.shelly_auth_pass || config.SHELLY_AUTH_PASS || config.password || config.PASSWORD || null;
                // Z-Wave input integration fields
                processed.zwaveMode = (config.zwave_mode || config.ZWAVE_MODE || 'direct').toString().trim().toLowerCase();
                processed.zwavePort = config.zwave_port || config.ZWAVE_PORT || null;
                processed.zwaveNodeId = parseInt(config.zwave_node_id || config.ZWAVE_NODE_ID || config.node_id || config.NODE_ID, 10) || undefined;
                processed.zwaveSensorType = (config.zwave_sensor_type || config.ZWAVE_SENSOR_TYPE || '').toString().trim().toLowerCase() || undefined;
                // Zigbee input integration fields
                processed.zigbeeMode = (config.zigbee_mode || config.ZIGBEE_MODE || 'direct').toString().trim().toLowerCase();
                processed.zigbeePort = config.zigbee_port || config.ZIGBEE_PORT || null;
                processed.zigbeeAdapter = (config.zigbee_adapter || config.ZIGBEE_ADAPTER || 'ember').toString().trim().toLowerCase();
                processed.zigbeeIeee = config.zigbee_ieee || config.ZIGBEE_IEEE || null;
                processed.zigbeeSensorType = (config.zigbee_sensor_type || config.ZIGBEE_SENSOR_TYPE || '').toString().trim().toLowerCase() || undefined;
                break;

            case 'relay':
                processed.controller = config.controller || config.CONTROLLER;
                processed.deviceId = config.device_id || config.DEVICE_ID || config.node_id || config.NODE_ID;
                break;
            ...
            default:
                this.logger.warn(`Unknown device type: ${deviceType} for device ${deviceName}`);
        }
